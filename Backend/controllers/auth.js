...
export const post_login = async (req, res) => {
  ...
  try {
    ...
    res.cookie("refreshToken", refreshToken, {path: '/', httpOnly: false,sameSite: 'none',maxAge:30 * 24 * 60 * 60 * 1000 , secure:true });
    res.cookie("token", token, { path: '/',httpOnly: false,sameSite: 'none',maxAge:30 * 24 * 60 * 60 * 1000 , secure:true });
